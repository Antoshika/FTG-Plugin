...
class ArtsMod(loader.Module):
    ...
    async def vjuhcmd(self, message):
        ...
        if not text:
            await message.edit('<b>Какой текст надо? :/</b>')
            return
        else:
            vjuh = ("<code>.∧＿∧\n"
                    "( ･ω･｡)つ━☆・*。\n"
    ...
    async def impscmd(self, message):
        ...
        imps = ['Этот чел не был Абобусом.', 'Этот чел был Абобусом.']
        imp = ("<code>.      　。　　　　•　    　ﾟ　　.      .     。\n"
               "　　.　　　.　　　  .　　　.　　　　　。　　   。　   .\n"
               "　.　　      。        ඞ   。　    .     　.　      •      .\n"
               f"•     {user.first_name} {choice(imps)} 。　   .\n"
               f"　 。     {randint(1, 5)} импостер(-ов)) осталось.　　　.　 　.\n"
               ",　　　　.　 .　　       .        •   •    。.\n"
               "。  •　   .   　ﾟ 　  •  　ﾟ .        .    　.</code>")
