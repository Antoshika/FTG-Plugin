...
class WebShotMod(loader.Module):
	...
	async def webshotcmd(self, message):
		...
		if not link:
			...
			if not reply:
				...
				return
			link = reply.raw_text
		await message.edit("<b>Фотаю твой сайтик...</b>")
		url = "https://webshot.deam.io/{}/?width=1920&height=1080?type=png"
		file = get(url.format(link))
		if not file.ok:
			await message.edit("<b>Эу осёл! Не верная ссылка или такой нету!</b>")
			return
		file = io.BytesIO(file.content)
		file.name = "screenshotweb(AET).png"
		file.seek(0)
	...
	async def fileshotcmd(self, message):
		"""Reply to file"""
		await message.edit("<b>Фотаю твой файлик...</b>")
		reply = await message.get_reply_message()
		if not reply:
			await message.edit("<b>гадэ файл?!.py</b>")
			return
		media = reply.media
		if not media:
			await message.edit("<b>гадэ файл?!.py</b>")
		...
		pygments.highlight(text, Python3Lexer(), ImageFormatter(font_name='DejaVu Sans Mono', line_numbers=True),
		                   'screenshotfile(AET).png')
		await message.client.send_file(message.to_id, 'screenshotfile(AET).png', force_document=True)
		os.remove("screenshotfile(AET).png")
		await message.delete()