...
class WhoIsMod(loader.Module):
    ...
    async def whoiscmd(self, message):
        ...
        await message.edit("<b>[FBR] Сбор информации...</b>")
...
async def get_info(user, message):
    ...
    photo = await message.client.download_profile_photo(user_id, str(user_id) + ".png", download_big=True)
    caption = (f"<b>ИНФОРМАЦИЯ О АМОГУСЕ:</b>\n\n"
